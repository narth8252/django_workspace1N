...
#JsonResponse이란, dict타입을 json으로 바꿔 응답하는 클래스
...
#데이터를 주는 경우
def getData(request):
    return JsonResponse({"name":"홍길동", "age":23, "phone":"010-000-0001"},
...
구축하는 방법 메모장에 써있음.
장고사이트구축방법_쌤.txt
HttpReponse - 그냥 텍스트로 응답할때 - 실제개발, 연습용
render - html문서+python데이터를 하나로 묶어서 던지면 새로운 html을 만들어서 클라이언트로 보낸다
        이 동작이 렌더링
JsonResponse -> 데이터를 json형태로 반환한다. ui/ux(html파트가 별도의 프레임워크로 만들어지는 경우 많다)

(base) C:\Windows\System32>conda activate mysite

(mysite) C:\Windows\System32>pip install mysqlclient
Requirement already satisfied: mysqlclient in c:\users\admin\anaconda3\lib\site-packages (2.2.7)

(mysite) C:\Windows\System32>
"""

...
# 성적 리스트 보기 (페이징 적용)
def list(request):
    scoreList = Score.objects.all().order_by('-id')  # 최신순 정렬
    ...
    page_obj = paginator.get_page(page_number)  # 해당 페이지의 객체만 추출
...
def update(request, id): #데이터저장
    ...
    if request.method == "POST":
        ...
        scoreModel.wdate = timezone.now()
        scoreModel.save()
        return redirect("score:score_view", id=scoreModel.id)
    else:
        form = ScoreForm(instance=scoreModel)
        print("name " , form)
    return render(request, 'score/score_write.html', {'form': form, 'modify': True, 'id': id})

# 웹서버 기본 주소가 http://127.0.0.1:8000/이라면,
# 목록 페이지:
# http://127.0.0.1:8000/score/list
# 상세 보기 (예: id=1):
# http://127.0.0.1:8000/score/view/1
# 작성 페이지:
# http://127.0.0.1:8000/score/write
# 수정 페이지 (예: id=1):
# http://127.0.0.1:8000/score/update/1
# 페이징 처리된 2페이지 목록
# http://127.0.0,1:8000/score/list?page=2
# score/가 기본 경로고, score_urls.py 안의 경로들이 뒤에 붙는 구조입니다.
# 이 주소들로 접속해서 기능 테스트해보시면 됩니다.